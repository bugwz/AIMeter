...
- `server`：API URL、フロント/バックのポート、プロキシ信頼
...
- `AIMETER_CRON_SECRET` と `AIMETER_ENDPOINT_SECRET` は env-only モードでは任意ですが、未設定の場合は該当の secret 認証エンドポイントは利用できません。
- データベースモードでは、`AIMETER_CRON_SECRET` と `AIMETER_ENDPOINT_SECRET` は初期化時のみ使用され、その後は DB 側の値で管理されます。
