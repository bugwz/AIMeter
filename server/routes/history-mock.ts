...
import { getMockUsageHistory, getAllMockUsageHistory } from '../mock/database.js';
import type { UsageRecordRow } from '../types/database.js';
  ...
  try {
    ...
    if (provider) {
      ...
      if (Number.isNaN(providerId)) {
        res.status(400).json({
          ...
          error: {
            ...
            message: 'Provider must be a numeric id',
          },
        });
        return;
      }
      ...
      data = Object.fromEntries(
        Array.from(allHistory.entries()).map(([providerId, records]) => [providerId, compactHistory(records, bucketMinutes)])
      );
    }
