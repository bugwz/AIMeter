...
import { getAllMockProviders, getMockLatestUsage } from '../mock/database.js';
...
router.get('/subscriptions', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const items: EndpointItem[] = selectedProviders.map((provider) => {
      const latestUsage = getMockLatestUsage(Number(provider.id));
